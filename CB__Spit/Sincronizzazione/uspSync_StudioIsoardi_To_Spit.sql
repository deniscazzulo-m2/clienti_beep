...
*
*/
CREATE PROCEDURE [uspSync_StudioIsoardi_To_Spit]
AS
SET NOCOUNT ON
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // STU_ISO-> SPIT  // Inizio sincronizzazione'
...
BEGIN TRAN
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			ItemValue
			, ItemDate)
		SELECT 'SINCRONIZZAZIONE // STU_ISO-> SPIT  // Tabella: ' + L.nome + ' // Azione: ' + L.azione + ' // Righe afflitte: ' + CAST(L.numero as VARCHAR), Cast(GETDATE() As smalldatetime)
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			...
			, ItemDate)
		VALUES(
			'SINCRONIZZAZIONE // STU_ISO-> SPIT  // Nessuna azione richiesta'
			, Cast(GETDATE() As smalldatetime));
	END

	IF ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0) > ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPIT].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
	BEGIN
		IF EXISTS(
			SELECT [ItemID]
		...
		BEGIN
			UPDATE [BpSPIT].dbo.[NumUniq] SET
				[ItemLast] = ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
			WHERE [ItemID] = 'MixAna1'
		END
		ELSE
		BEGIN
			INSERT [BpSPIT].dbo.[NumUniq]([ItemID], [ItemLast])
			SELECT TOP 1 [ItemID], [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'
		END
	END
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // STU_ISO-> SPIT  // Fine sincronizzazione'
	, Cast(GETDATE() As smalldatetime));