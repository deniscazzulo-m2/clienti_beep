...
*
*/
CREATE PROCEDURE [uspSync_SpitGest_To_StudioIsoardi]
AS
SET NOCOUNT ON
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT_GEST -> STU_ISO // Inizio sincronizzazione'
...
BEGIN TRAN
	...
	OUTPUT
	 ...
	 MERGE [BpSTUDIOISOARDI].[dbo].[MixAnaAlias] T USING (
	    SELECT
	        *
	    FROM
	        [BpSPITGEST].[dbo].[MixAnaAlias]
	    WHERE
	        [ItemType] = '1'
	) S ON RTRIM(LTRIM(ISNULL(T.[ItemID],''))) = RTRIM(LTRIM(ISNULL(S.[ItemID],'')))
	AND RTRIM(LTRIM(ISNULL(T.[ItemType],''))) = RTRIM(LTRIM(ISNULL(S.[ItemType],'')))
	...
	OUTPUT
	  ...
	  MERGE [BpSTUDIOISOARDI].[dbo].[MixAnaSedi] T USING (
	    SELECT
	        *
	    FROM
	        [BpSPITGEST].[dbo].[MixAnaSedi]
	    WHERE
	        [ItemType] = '1'
	) S ON RTRIM(LTRIM(ISNULL(T.[ItemID],''))) = RTRIM(LTRIM(ISNULL(S.[ItemID],'')))
	AND RTRIM(LTRIM(ISNULL(T.[ItemType],''))) = RTRIM(LTRIM(ISNULL(S.[ItemType],'')))
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			ItemValue
			, ItemDate)
		SELECT 'SINCRONIZZAZIONE // SPIT_GEST -> STU_ISO // Tabella: ' + L.nome + ' // Azione: ' + L.azione + ' // Righe afflitte: ' + CAST(L.numero as VARCHAR), Cast(GETDATE() As smalldatetime)
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			...
			, ItemDate)
		VALUES(
			'SINCRONIZZAZIONE // SPIT_GEST -> STU_ISO // Nessuna azione richiesta'
			, Cast(GETDATE() As smalldatetime));
	END

	IF ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPITGEST].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0) > ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
	BEGIN
		IF EXISTS(
			SELECT [ItemID]
		...
		BEGIN
			UPDATE [BpSTUDIOISOARDI].dbo.[NumUniq] SET
				[ItemLast] = ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPITGEST].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
			WHERE [ItemID] = 'MixAna1'
		END
		ELSE
		BEGIN
			INSERT [BpSTUDIOISOARDI].dbo.[NumUniq]([ItemID], [ItemLast])
			SELECT TOP 1 [ItemID], [ItemLast] FROM [BpSPITGEST].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'
		END
	END
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT_GEST -> STU_ISO // Fine sincronizzazione'
	, Cast(GETDATE() As smalldatetime));