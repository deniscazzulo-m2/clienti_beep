...
*
*/
CREATE PROCEDURE [uspSync_Spit_To_StudioIsoardi]
AS
SET NOCOUNT ON
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT -> STU_ISO // Inizio sincronizzazione'
...
BEGIN TRAN
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			ItemValue
			, ItemDate)
		SELECT 'SINCRONIZZAZIONE // SPIT -> STU_ISO // Tabella: ' + L.nome + ' // Azione: ' + L.azione + ' // Righe afflitte: ' + CAST(L.numero as VARCHAR), Cast(GETDATE() As smalldatetime)
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			...
			, ItemDate)
		VALUES(
			'SINCRONIZZAZIONE // SPIT -> STU_ISO // Nessuna azione richiesta'
			, Cast(GETDATE() As smalldatetime));
	END

	IF ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPIT].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0) > ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
	BEGIN
		IF EXISTS(
			SELECT [ItemID]
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT -> STU_ISO // Fine sincronizzazione'
	, Cast(GETDATE() As smalldatetime));