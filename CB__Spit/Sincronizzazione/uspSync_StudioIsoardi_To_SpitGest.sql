...
*
*/
CREATE PROCEDURE [uspSync_StudioIsoardi_To_SpitGest]
AS
SET NOCOUNT ON
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // STU_ISO-> SPIT_GEST  // Inizio sincronizzazione'
...
BEGIN TRAN
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			ItemValue
			, ItemDate)
		SELECT 'SINCRONIZZAZIONE // STU_ISO-> SPIT_GEST  // Tabella: ' + L.nome + ' // Azione: ' + L.azione + ' // Righe afflitte: ' + CAST(L.numero as VARCHAR), Cast(GETDATE() As smalldatetime)
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			...
			, ItemDate)
		VALUES(
			'SINCRONIZZAZIONE // STU_ISO-> SPIT_GEST  // Nessuna azione richiesta'
			, Cast(GETDATE() As smalldatetime));
	END

	IF ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSTUDIOISOARDI].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0) > ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPITGEST].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
	BEGIN
		IF EXISTS(
			SELECT [ItemID]
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // STU_ISO-> SPIT_GEST  // Fine sincronizzazione'
	, Cast(GETDATE() As smalldatetime));