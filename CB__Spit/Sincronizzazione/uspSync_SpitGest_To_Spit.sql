...
*
*/
CREATE PROCEDURE [uspSync_SpitGest_To_Spit]
AS
SET NOCOUNT ON
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT_GEST-> SPIT  // Inizio sincronizzazione'
...
BEGIN TRAN
	...
	MERGE [BpSPIT].[dbo].[MixAna] T USING (
	    SELECT
	        *
	    FROM
	...
	MERGE [BpSPIT].[dbo].[MixAnaRif] T USING (
	    SELECT
	        *
	    FROM
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			ItemValue
			, ItemDate)
		SELECT 'SINCRONIZZAZIONE // SPIT_GEST-> SPIT  // Tabella: ' + L.nome + ' // Azione: ' + L.azione + ' // Righe afflitte: ' + CAST(L.numero as VARCHAR), Cast(GETDATE() As smalldatetime)
	...
	BEGIN
		INSERT INTO dbo.[ZLogSync_Pers](
			...
			, ItemDate)
		VALUES(
			'SINCRONIZZAZIONE // SPIT_GEST-> SPIT  // Nessuna azione richiesta'
			, Cast(GETDATE() As smalldatetime));
	END

	IF ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPITGEST].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0) > ISNULL((SELECT TOP 1 [ItemLast] FROM [BpSPIT].[dbo].[NumUniq] WHERE [ItemId] = 'MixAna1'),0)
	BEGIN
		IF EXISTS(
			SELECT [ItemID]
...
INSERT INTO dbo.[ZLogSync_Pers](
	...
	, ItemDate)
VALUES(
	'SINCRONIZZAZIONE // SPIT_GEST-> SPIT  // Fine sincronizzazione'
	, Cast(GETDATE() As smalldatetime));