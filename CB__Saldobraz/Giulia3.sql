...
Declare @sFields_DOC varchar(2048) = 'DOC.DocTip,DOC.DocNum,DOC.DocDta'

Declare @sFields_DOCLIN varchar(1024) = 'LIN.LinNum,LIN.PrdCod,LIN.PrdParams,LIN.PrdDes,LIN.PrdUm,LIN.PrdQta,LIN.LinRif,LIN.Tm,PRD.GrpCod'

Declare @sFields_ANA varchar(1024) = 'ANA.ItemID,ANA.ItemDes'

Declare @sSql_DOC varchar(1024) = 'Declare cCursor_2 Cursor FAST_FORWARD For'
								+ ' Select Cast(IsNull(Cast({FIELD} as varchar(256)), '''') As varchar(256)) '
								+ ' From dbo.[' + @sTable_DOC + '] DOC'
								+ ' Where Uniq = {UNIQ}'

Declare @sSql_DOCLIN varchar(1024) = 'Declare cCursor_2 Cursor FAST_FORWARD For'
								+ ' Select Cast(IsNull(Cast({FIELD} as varchar(256)), '''') As varchar(256)) '
								+ ' From dbo.[' + @sTable_DOCLIN + '] LIN Left Join dbo.[PrdAna] PRD On LIN.[PrdCod] = PRD.[PrdCod]'

Declare @sSql_ANA varchar(1024) = 'Declare cCursor_2 Cursor FAST_FORWARD For'
								+ ' Select IsNull(Cast({FIELD} as varchar(256)), '''') '
								+ ' From dbo.[MixAna] ANA'
								+ ' Where ItemType = ' + Cast(@iAnaType As char(1))
								+ '	   And ItemID = ''{ITEMID}'' '
...
Execute(@sSql_1)
Open [cCursor_2]
Fetch Next From [cCursor_2] Into @sAnaCod
Close [cCursor_2]
Deallocate [cCursor_2]
...
BEGIN
	...
	Begin
		Set @sSql_1 = @sSql
		Set @sSql_1 = Replace(@sSql_1, '{FIELD}', @sField)
		Set @sSql_1 = Replace(@sSql_1, '{ITEMID}', @sAnaCod)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			Begin
				Set @sSql = @sSql_DOCLIN
				Set @sSql = Replace(@sSql, '{FIELD}', @sField)
				Set @sSql = Replace(@sSql, '{UNIQ}', @sUniqLin)
